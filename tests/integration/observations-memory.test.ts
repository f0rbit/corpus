...
describe("observations integration - memory backend", () => {
	...
	describe("type validation", () => {
		...
		it("rejects invalid enum value", async () => {
			const result = await observations.put(entity_type, {
				source: create_pointer("docs", "v1"),
				content: { name: "Test", type: "invalid" as never, mentions: 1 },
			});
			expect(result.ok).toBe(false);
			if (!result.ok) {
				expect(result.error.kind).toBe("validation_error");
	...
	describe("query filtering", () => {
		...
		it("respects limit of 2", async () => {
			const results: unknown[] = [];
			for await (const obs of observations.query({ limit: 2 })) {
				results.push(obs);
			}
			expect(results.length).toBe(2);
		});
		...
		it("query_meta respects filters", async () => {
			const results: unknown[] = [];
			for await (const meta of observations.query_meta({ source_store: "articles" })) {
				results.push(meta);
			}
			expect(results.length).toBe(1);
		});
	...
	describe("optional fields", () => {
		...
		it("stores source with path and span together", async () => {
			...
			expect(get_result.ok).toBe(true);
			if (get_result.ok) {
				expect(get_result.value.source.path).toBe("$.text");
				expect(get_result.value.source.span).toEqual({ start: 50, end: 150 });
			}
		});
		...
		it("omits undefined optional fields", async () => {
			...
			expect(get_result.ok).toBe(true);
			if (get_result.ok) {
				expect(get_result.value.confidence).toBeUndefined();
				expect(get_result.value.observed_at).toBeUndefined();
				expect(get_result.value.derived_from).toBeUndefined();
				expect(get_result.value.source.path).toBeUndefined();
				expect(get_result.value.source.span).toBeUndefined();
			}
		});
	});
	...
	describe("version_filter", () => {
		...
		it("filters with sync function", async () => {
			const results: unknown[] = [];
			for await (const obs of observations.query({
		...
		it("filters with async function", async () => {
			const results: unknown[] = [];
			for await (const obs of observations.query({
		...
		it("excludes non-matching versions", async () => {
			const results: unknown[] = [];
			for await (const obs of observations.query({ version_filter: new Set(["nonexistent"]) })) {
				results.push(obs);
			}
			expect(results.length).toBe(0);
		});

		it("returns all when no filter provided", async () => {
			const results: unknown[] = [];
			for await (const obs of observations.query({})) {
				results.push(obs);
			}
		...
		it("query_meta respects version_filter with Set", async () => {
			const results: unknown[] = [];
			for await (const meta of observations.query_meta({ version_filter: new Set(["v2", "v3"]) })) {
				results.push(meta);
			}
			expect(results.length).toBe(2);
		});

		it("query_meta respects version_filter with function", async () => {
			const results: unknown[] = [];
			for await (const meta of observations.query_meta({
				version_filter: (_store_id, version) => version === "v1",
			})) {
				results.push(meta);
			}
			expect(results.length).toBe(1);
		...
		it("function receives store_id correctly", async () => {
			await observations.put(simple_type, {
				source: create_pointer("other", "v1"),
				content: { value: "other-store" },
			});

			const results: unknown[] = [];
			for await (const obs of observations.query({
				version_filter: (store_id, _version) => store_id === "docs",
			})) {
				results.push(obs);
			}
			expect(results.length).toBe(3);
	...
	describe("timestamps", () => {
		...
		it("preserves created_at on retrieval", async () => {
			...
			expect(get_result.ok).toBe(true);
			if (get_result.ok) {
				expect(get_result.value.created_at.toISOString()).toBe(result.value.created_at.toISOString());
			}
		});

		it("created_at differs from observed_at", async () => {
			const observed_at = new Date("2024-01-01");
			const result = await observations.put(simple_type, {
				source: create_pointer("docs", "v1"),
				content: { value: "test" },
				observed_at,
			...
			expect(result.value.created_at.getTime()).not.toBe(observed_at.getTime());
			expect(result.value.observed_at?.toISOString()).toBe(observed_at.toISOString());
		});
	});
	...
	describe("observation id format", () => {
		it("generates unique ids", async () => {
			...
			for (let i = 0; i < 10; i++) {
				const result = await observations.put(simple_type, {
					source: create_pointer("docs", `v${String(i)}`),
					content: { value: `test${String(i)}` },
				});
				expect(result.ok).toBe(true);
				if (result.ok) {
	...
	describe("created_at filtering", () => {
		...
		it("filters by created_after", async () => {
			...
			for await (const obs of observations.query({ created_after: cutoff })) {
				filtered.push(obs);
			}
		...
		it("filters by created_before", async () => {
			const cutoff = new Date(Date.now() - 25);
			const filtered: unknown[] = [];
			for await (const obs of observations.query({ created_before: cutoff })) {
				filtered.push(obs);
			}
			expect(filtered.length).toBe(1);
		});
