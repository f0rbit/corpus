...
export function create_store<T>(backend: Backend, definition: StoreDefinition<string, T>): Store<T> {
	...
	async function get_handle_impl(
		version: string,
	): Promise<Result<{ meta: SnapshotMeta; handle: SnapshotHandle<T> }, CorpusError>> {
	...
	async function get_latest_handle_impl(): Promise<
		Result<{ meta: SnapshotMeta; handle: SnapshotHandle<T> }, CorpusError>
	> {
	...
	async function put_stream_impl(
	...
	function make_builder<Stores extends Record<string, Store<any>>>(): CorpusBuilder<Stores> {
		return {
			...
			build(): Corpus<Stores> {
				if (!backend) {
					throw new Error(
						"create_corpus().build() called without a backend — chain .with_backend(<backend>) before .build()",
					);
				}
	...
	async function transaction_impl<R>(
		...
		const handle: TransactionHandle = {
			async put<T>(store: Store<T>, data: T, opts?: PutOpts): Promise<Result<SnapshotMeta, CorpusError>> {
				...
				let existing_data_key: string | undefined = find_buffered_by_hash(store.id, content_hash)?.data_key;
		...
		if (!body_result.ok) {
			...
			return err({
				...
				reason: "returned_err",
				cause: corpus_error_to_native(body_result.error),
			});
		}

		// commit phase
		const commit_result = b.apply_batch ? await b.apply_batch(ops) : await sequential_apply_with_compensation(b, ops);
