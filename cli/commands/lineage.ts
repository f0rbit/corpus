...
export const lineage_command: Command = {
	...
	async run(ctx: CommandContext): Promise<Result<void, CorpusError>> {
		try {
			...
			// Get the root snapshot metadata
			...
			const build_tree = (node: LineageNode): TreeNode => {
				const label = node.missing ? `${node.store}/${node.version} (missing)` : `${node.store}/${node.version}`;
				...
				for (const parent of node.parents) {
					...
					if (parent_node && node.depth < max_depth) {
						children.push(build_tree(parent_node));
					}
				}
			...
			const tree = build_tree(root_node);
			const tree_lines = render_tree(tree);
			...
			// JSON output
			const json_nodes = nodes.map((node) => ({
				store: node.store,
