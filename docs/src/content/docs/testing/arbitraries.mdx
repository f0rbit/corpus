...
<Aside type="caution">
  `CORPUS_ERROR_BRAND` is the canonical brand for `CorpusError` — corpus registers generators for all its variants, and downstream packages add failure cases against the same imported symbol. Minting your own `Symbol("CorpusError")` creates a parallel registry entry nothing else can see. Your own package's error union gets its own brand.
</Aside>
