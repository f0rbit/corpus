...
description: Extended utilities for Result types, concurrency control, hashing, and versioning in Corpus.
---
...
<Tabs defaultValue="existing" client:load>
<TabList client:load>
<Tab value="existing" client:load>From existing Result</Tab>
<Tab value="async" client:load>From async operation</Tab>
<Tab value="try" client:load>From try block</Tab>
<Tab value="fetch" client:load>From fetch</Tab>
</TabList>
<TabPanel value="existing" client:load>
```typescript
// From a sync Result
...
<TabPanel value="async" client:load>
```typescript
// From Promise<Result>
...
<TabPanel value="try" client:load>
```typescript
// Wrap throwing code
...
<TabPanel value="fetch" client:load>
```typescript
// Wrap fetch call
...
<Tabs defaultValue="composable" client:load>
<TabList client:load>
<Tab value="composable" client:load>Composable</Tab>
<Tab value="verbose" client:load>Verbose Alternative</Tab>
</TabList>
<TabPanel value="composable" client:load>
```typescript
...
<TabPanel value="verbose" client:load>
```typescript
...
<Tabs defaultValue="composable2" client:load>
<TabList client:load>
<Tab value="composable2" client:load>Composable</Tab>
<Tab value="verbose2" client:load>Verbose Alternative</Tab>
</TabList>
<TabPanel value="composable2" client:load>
```typescript
...
<TabPanel value="verbose2" client:load>
```typescript
// Without composition
...
These two helpers underpin Corpus's content-addressable storage and lexicographic version ordering. Both are exposed for callers that want to pre-compute hashes (e.g. to short-circuit a `put`) or generate compatible version strings outside of a `put` call.

### `compute_hash(data)`

Computes the SHA-256 hash of binary data and returns it as a lowercase 64-character hex string.
...
**Signature:**
```typescript
function compute_hash(data: Uint8Array): Promise<string>
```

Use case: pre-compute the hash of a payload and check the store for an existing snapshot with that `content_hash` before issuing a write — useful when the source bytes are large and you want to avoid even encoding through the codec on a cache hit.

---

### `generate_version()`

Returns a unique, lexicographically sortable version string. Format is a base64url-encoded big-endian timestamp, with an optional `.N` suffix when multiple versions are generated inside the same millisecond.

```typescript
import { generate_version } from '@f0rbit/corpus'

const v1 = generate_version() // => 'AZJx4vM'
const v2 = generate_version() // => 'AZJx4vM.1' (same ms)
const v3 = generate_version() // => 'AZJx4vN' (next ms)
...
You normally don't call this directly — `store.put()` generates a version automatically. Use it when constructing `SnapshotMeta` manually (e.g. when migrating data into Corpus from another system) or when you need a version string ahead of the write to reference it elsewhere.
